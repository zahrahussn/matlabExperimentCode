...
%    calibrationMethod='bitStealing';
    ...
    thresholdguess=[0.000009, 0.00012]; % low noise guess reduced, july 2018.
...
%         constimrec(kk).appspec=nv(kk);	% attached noise variance to app-specific field
   ...
   sidnum = 0;
    ...
    screenFrequency = [60 70 75 80 85];% 85
    screenPxSize = [24 32]; %32
  ...
  % max(max(finalStim))
	...
	if (response > 0)
		...
		if correct
            ...
            %sound(corrsnd, SND_RATE);
		else
			% sound(wrongsnd);
			%err=Snd('Play',wrongsnd,SND_RATE);
        %    sound(wrongsnd, SND_RATE);
		end;
