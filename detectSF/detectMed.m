...
%    calibrationMethod='bitStealing';
    ...
    defarg('detectMedTEXTfolder','detectMedTEXTfiles');	% N.B. make sure this folder exists within the folder containing 'trialslab'!
    defarg('detectMedMATfolder','detectMedMATfiles');		% N.B. make sure this folder exists within the folder containing 'trialslab'!

    % set the paths
    disp('setting path names...');
    mfilename='detectMed'; mainfile = [mfilename,'.m'];
    ...
    textdatadir=[mainpath,detectMedTEXTfolder];
    matdatadir=[mainpath,detectMedMATfolder];
    cd(mainpath);
    disp(mainpath);
    ...
    thresholdguess=[1.5*0.00003, 1.5*0.00012]; %Sept 26, 2018; low noise guess raised; med guesses = 2x low guess
...
%         constimrec(kk).appspec=nv(kk);	% attached noise variance to app-specific field
   ...
   sidnum = 0;
    ...
    load theStimulus5 % fixed to avoid experimenter error
   ...
   % stimNames=fieldnames(eval(['theStimulus', num2str(stimset)]));
    ...
    for kk=1:numStim
        ...
        theStim(kk).stim=eval(['theStimulus5.',char(stimNames(kk))]); % modified sept 2018 to fix stimulus to med textures
    end;
