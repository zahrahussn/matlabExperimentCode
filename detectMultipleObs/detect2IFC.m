    %% 2IFC detection; ZH Oct 2019
    %% interval is selected randomly from 1 or 2 on each trial; could result in many trials with same interval (i.e., not sampling without replacement)
 ...
 %   calibrationMethod='psychophysical';
    ...
    mfilename='detectMultObs'; mainfile = [mfilename,'.m'];
    eval(['mainpath=which(',QuoteString(mainfile),');']);
    %mainpath=strrep(mainpath,'experimentCode', 'detectMultipleObs'); % modified sept 10, 2018 for github rearrangement
    ...
    for kk=1:numnz        
        ...
        values(kk,:)=val(kk,:); % removed 0-contrast values for the 2IFC task; oct 20191
...
%     bit-stealing and using tim's calibration.
   ...
   % for z = 1:2
    ...
    z=randomcondition(2); % low or high noise; blocked
    condcount=0;
    
    while ((alldone==0)&(quitflag==0))
        int=randomcondition(2); % interval 1 or 2  
        ...
        [noise,nzseed]=noise2d(stimpix,nzvar,cmin,cmax,0);
   ...
   %%%  2IFC contrasts 
       tmpStim1=theStim(curStim).stim*sqrt(stim1var); % this formula works because the faces have a variance of 1
       Stim1=noise+tmpStim1; % add noise to texture

       tmpStim2=theStim(curStim).stim*sqrt(stim2var);
       Stim2=noise+tmpStim2; % add noise to texture
        ...
        Screen('DrawTexture', w, Stim1, srcRect, dstRect);  % actually putting the image on the screen
        ...
        timeStim=time0+.2;
        while(time0<timeStim)
        Screen('DrawTexture', w, Stim2, srcRect, dstRect);  % actually putting the image on the screen
        Screen('FrameRect', w, 10, boxrect, 1); % putting the frame around the stim on the screen
        [time0]= Screen('Flip', w); % making it appear
        end
        stimofftime=GetSecs;
	...
	if (response>0)
		data(curtrial,:)=[curtrial,condID,nzvar,curStimVariance,curStim,int,response,correct,rlatency,nzseed']; % update data matrix
...
%     csvwrite(['pcorrect_',num2str(sidnum),'_',num2str(trialspervalue(groupID)),'trials_',strcat(stimulusSet{sstimset}),'.csv'],pcorrect)
    ...
    count=fprintf(fid,'\n%s\n',['DATA']);
    count=fprintf(fid,'%s \t %s \t %s \t %s \t %s \t %s \t %s \t %s \t %s \n','trial #','conditionID','nz var','stim var','stim ID', 'interval', 'response','correct','RT(sec)');

    [numtrials,tmp0]=size(data);

    for kk=1:numtrials fprintf(fid,'%i \t %i \t %8.6f \t %8.6f \t %i \t %i \t %i \t %6.2f \t %i\n',data(kk,1:9)); end
    fclose(fid);
