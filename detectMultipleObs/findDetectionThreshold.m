...
defarg('trialspervalue',[1,10,40]); % # of trials per contrast level
%value =  0.0001986;
    ...
    for t = 1:trialspervalue(groupID)
        ...
        switch(calibrationMethod)
            ...
            while(time0<timeStim)
                Screen('DrawTexture', w, Stim1, srcRect, dstRect);  % put the image on the screen
            ...
            if (response>0)
                data(curtrial,:)=[curtrial,b,t,nc,curStim,int,response,correct,rlatency]; % update data matrix
...
Screen('DrawText',w,'Phase one done.',fixPntX,fixPntY);
Screen('Flip',w);
WaitSecs(2);
Screen('CloseAll');
ShowCursor(0);
...
count=fprintf(fid,'%s\n',['Data were fit with a weibull function: Gamma = ',num2str(psiParamsFit(3)),'; Delta = ',num2str(psiParamsFit(4))]);
count=fprintf(fid,'%s\n',['Detection threshold was estimated as ',num2str(db2contrast(psiParamsFit(1)))]);
...
count=fprintf(fid,'\n%s\n','DATA');
count=fprintf(fid,'%s \t %s \t %s \t %s \t %s \t %s \t %s \t %s \t %s \n','trial #', 'block', 'blockTrial','conditionID','stim ID', 'interval', 'response','correct','RT(sec)');

[numtrials,tmp0]=size(data);

for kk=1:numtrials, fprintf(fid,'%i \t %i \t %i \t %i \t %8.6f \t %8.6f \t %i \t %i \t %i \n',data(kk,1:9)); end
fclose(fid);
