...
defarg('trialspervalue',[2,100]); % # of trials per contrast level
value =  0.0001986;
...
Screen('DrawText',w,'All done!',fixPntX/2,fixPntY);
Screen('Flip',w);
[secs, keyCode, deltaSecs] = KbWait;
while keyCode(KbName('space'))~=1;
    [secs, keyCode, deltaSecs] = KbWait;
...
k=1;
while found
    matfname=['grp',num2str(trialspervalue(groupID)), '_', num2str(numObs), 'obs','_ID',sidnum,'_TexturesB', '_N',num2str(k)];
...
textfname=['grp',num2str(trialspervalue(groupID)), '_', num2str(numObs), 'obs','_ID',sidnum,'_TexturesB'];
