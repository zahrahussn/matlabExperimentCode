...
try
    ...
    fprintf('\n(calibration)$ you may new change the spatial and temporal resolution of the display\n')
...
%	colorNumber=5:5:255;
    ...
    % deal with problem of taking log of zero:
    tol = 1e-4;
    tmp0=find(lumData(:,5)==0);
    lumData(tmp0,5)=tol;
    tmp0=find(lumData(:,4)==0);
    lumData(tmp0,4)=tol;
    
    logErr=log10(lumData(:,5)./lumData(:,4));
    [y,i]=sort(lumData(:,4));
    figure(10);plot(lumData(i,4),logErr(i),'ko-');
    title('all luminance values');xlabel('Luminance');ylabel('Log Error');
    
    tmp=find(lumData(:,4)>1);
    lumtmp=lumData(tmp,4);
    xlumtmp=lumData(tmp,5);
    logErr=log10(xlumtmp./lumtmp);
    [y,i]=sort(lumtmp);
    figure(11);plot(lumtmp(i),logErr(i),'ko-');
    title('luminance values above 1 cd/m^2');xlabel('Luminance');ylabel('Log Error');
