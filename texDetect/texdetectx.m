% function pl2012
    ...
    defarg('exptname','learning_trials');	% the name of t §he experiment
    ...
    defarg('texdetectxTEXTfolder','texdetectxTEXTfiles');	% N.B. make sure this folder exists within the folder containing 'trialslab'!
    defarg('texdetectxMATfolder','texdetectxMATfiles');		% N.B. make sure this folder exists within the folder containing 'trialslab'!

    % set the paths
    disp('setting path names...');
    mfilename='texdetectx'; mainfile = [mfilename,'.m'];
    eval(['mainpath=which(',QuoteString(mainfile),');']);
    mainpath = mainpath(1:end-length(mainfile));
    textdatadir=[mainpath,texdetectxTEXTfolder];
    matdatadir=[mainpath,texdetectxMATfolder];
    cd(mainpath);
    disp(mainpath);
...
%         constimrec(kk).appspec=nv(kk);	% attached noise variance to app-specific field
    ...
    stimulusSet = {'Faces_A','Faces_B','Textures_A','Textures_B'};
   ...
   finalStim=stim+tmpStim; % add noise to face
    ...
    while (gotresponse==0)
        [keyIsDown,secs,keyCode] = KbCheck;
                if keyIsDown==1 && keyCode(presentKey)==1
                    gotresponse=1; response = 2;
                elseif  keyIsDown==1 && keyCode(absentKey)==1;
                        gotresponse=1; response = 1;
                elseif  keyIsDown==1 && keyCode(escapeKey)==1
                        gotresponse=1;
                        Screen('CloseAll');
                        quitflag=1
                end
